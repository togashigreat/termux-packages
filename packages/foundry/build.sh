...
termux_step_pre_configure() {
	...
	local git_patch="$TERMUX_PKG_BUILDER_DIR/fix-git-error.diff"
	patch -p1 -d "$TERMUX_PKG_SRCDIR" < "$git_patch"
	...
	# patch cc-rs to override CFLAGS
	...
	# patch cc_builder.rs to avoid adding LDFLAGS `-march=armv7-a`
